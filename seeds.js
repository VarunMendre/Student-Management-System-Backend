...
const seedDatabase = async () => {
    ...
    try {
        ...
        const btechRes = await pool.query("SELECT id FROM courses WHERE course_code = 'BCS101'");
        const btechId = btechRes.rows[0].id;
        const mbaRes = await pool.query("SELECT id FROM courses WHERE course_code = 'MBA202'");
        ...
        const mbaFees = [
            ['Tuition Fee', 120000.00],
            ['Library Fee', 5000.00],
            ['Activity Fee', 5000.00]
        ];

        await pool.query("BEGIN");
        await pool.query("DELETE FROM course_fees WHERE batch_id = $1", [btechBatchId]);
        ...
        await pool.query("DELETE FROM course_fees WHERE batch_id = $1", [mbaBatchId]);
        ...
        const sampleStudents = [
            ['Rahul Sharma', 'rahul.sharma@example.com', '9876543210', '9123456789', null, null, csitId, btechId, btechBatchId, 'OBC', 'Male'],
            ['Priya Patil', 'priya.patil@example.com', '9876543211', '9123456788', 'PRN2025002', null, csitId, btechId, btechBatchId, 'General', 'Female'],
            ['Amit Deshmukh', 'amit.deshmukh@example.com', '9876543212', '9123456787', null, null, mgmtId, mbaId, mbaBatchId, 'SC', 'Male']
        ];
        ...
        if (enrolledStudentIds.length > 0) {
            ...
            if (ledgerRes.rows.length > 0) {
                ...
                if (existingTxn.rows.length === 0) {
                    ...
                    `, [rahulId, ledgerId, 25000.00, 'UPI', 'UTR987654321', receiptNumber, 'First installment FY', '2026-04-10', 'admin']);
