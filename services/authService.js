...
const frontendOrigins = String(process.env.FRONTEND_URL || "")
  ...
  .map(normalizeOrigin)
  .filter(Boolean);
const IS_HTTPS_FRONTEND = frontendOrigins.some((origin) =>
  origin.startsWith("https://"),
);
const requestedSameSite = String(process.env.COOKIE_SAME_SITE || "")
  .trim()
  .toLowerCase();
...
const updateCurrentUser = async (userId, { name, email, contact_number }) => {
  ...
  if (updatedUser.student_id) {
    try {
      ...
      console.error(
        `Failed to sync student ${updatedUser.student_id}:`,
        syncError,
      );
...
const resetUserPassword = async (userId, { currentPassword, newPassword }) => {
  ...
  if (updatedUser?.student_id) {
    await userModel.updateStudentPasswordChangedStatus(
